...
export const reportingusers = (req, res) => {
    console.log(req.body)
    const { option } = req.body
    let search_query;
    switch (option) {
        case 'Employee':
            search_query = `select users, profile_pic, concat(first_name,' ',last_name) as fullname,employee_id,designation from reportingstructure  inner join usermanagement on employee_id = users; `
            break;
        case 'Reporting Head':
            search_query = `select distinct(reporting_head), profile_pic, concat(first_name,' ',last_name) as fullname,employee_id,designation from reportingstructure  inner join usermanagement on employee_id = reporting_head; `
            break
...
export const getreportingstructuredata = (req, res) => {
    console.log(req.body)
    const { option, emp_id } = req.body
    let search_query;
    const search_value=[emp_id]
    switch (option) {

        case 'Employee':
            search_query = `select profile_pic, concat(first_name,' ',last_name) as fullname,employee_id,designation from reportingstructure  inner join usermanagement on employee_id = reporting_head where users=?; `

            break;
        case 'Reporting Head':
            search_query = `select profile_pic, concat(first_name,' ',last_name) as fullname,employee_id,designation from reportingstructure  inner join usermanagement on employee_id = users where reporting_head=?; `
            break
